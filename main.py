...
def search_products(
    ...
    with get_db() as conn:
        ...
        cursor.execute("""
            SELECT * FROM products 
            WHERE name LIKE %s OR brand LIKE %s OR upc LIKE %s
            ORDER BY 
...
def add_scan(session_id: str, scan_data: ScanCreate, user_id: str = Depends(get_current_user)):
    ...
    with get_db() as conn:
        ...
        cursor.execute("SELECT COUNT(*) as cnt FROM scans WHERE session_id = %s", (session_id,))
        total = cursor.fetchone()["cnt"]
...
def pen_capture(capture_data: PenCaptureRequest, user_id: str = Depends(get_current_user)):
    ...
    with get_db() as conn:
        ...
        cursor.execute("SELECT COUNT(*) as cnt FROM scans WHERE session_id = %s", (capture_data.session_id,))
        bottle_number = cursor.fetchone()["cnt"]
